...
and so the wave is able to travel continuously around the arena; in discontiguous seating arrangements, the wave can instead reflect
back and forth through the crowd. When the gap in seating is narrow, the wave can sometimes pass through it. 
...
const wave = (str) => {
    ...
    for (let i = 0; i <= str.length - 1; i++) {
        ...
        if (strToArray[i] != ` `) {
            arrOut.push(strToArray.toString());
        }
    ...
    for (let i = 0; i <= arrOut.length - 1; i++) {
        let strOut = arrOut[i];
        for (let j = 0; j <= strOut.length - 1; j++) {
